...
//
//     For every active set, runs the full estimation engine and UPSERTS rows
...
function refreshLiveEstimates() {
  ...
  // ── Load graded sales for normalization ────────────────────────────────
  const gradedSalesData = sbGet_('sales',
    'select=player,variant,price,date' +
    '&afa_grade=not.is.null' +
    '&or=(status.neq.invalid,status.is.null)'
  );
  const gradedNormMap = buildGradedNormMap(gradedSalesData || []);
  applyGradingNorm(allData.products, gradedNormMap);
  console.log(`  Grading normalization: ${gradedSalesData.length} graded sales processed`);
...
function createSnapshotTriggers() {
  const MANAGED = ['takeEstimateSnapshots', 'resolveSnapshots', 'refreshLiveEstimates'];
